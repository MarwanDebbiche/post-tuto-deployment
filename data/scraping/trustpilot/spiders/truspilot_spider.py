...
class Pages(scrapy.Spider):
    ...
    def parse(self, response):
        for href in response.css('li.category-menu-list-item a::attr(href)'):
            request = response.follow(href, callback=self.parse_websites_page)
            request.meta['url_category'] = href.extract()
            yield request 
